        ...
        case SENSOR_QXGA_30FPS_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 0.5))
            {
                if(WDR_MODE_2To1_LINE == genSensorMode)
                {
                    ...
                    u32VMAX = (u32VMAX > 0x1FFFF) ? 0x1FFFF : u32VMAX;
    ...
    AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32FullLines = (u32FullLines > 0x1FFFF) ? 0x1FFFF : u32FullLines;
    gu32FullLines = u32FullLines;

    if (WDR_MODE_NONE == genSensorMode)
    {
        g_stSnsRegsInfo.astSspData[5].u32Data = (gu32FullLines & 0xFF);
        g_stSnsRegsInfo.astSspData[6].u32Data = ((gu32FullLines & 0xFF00) >> 8);
        g_stSnsRegsInfo.astSspData[7].u32Data = ((gu32FullLines & 0x10000) >> 16);
    }
    else
    {
        g_stSnsRegsInfo.astSspData[8].u32Data = (gu32FullLines & 0xFF);
        g_stSnsRegsInfo.astSspData[9].u32Data = ((gu32FullLines & 0xFF00) >> 8);
        g_stSnsRegsInfo.astSspData[10].u32Data = ((gu32FullLines & 0x10000) >> 16);
    }
...
#if FIXED_RHS1_MODE
            u32SHS2 = gu32FullLines - u32LongIntTime;
...
static HI_U32 gain_table[511]=
{
    1024,  1036,  1048,  1060,  1072,  1085,  1097,  1110,  1123,  1136,  1149,  1162,  1176,  1189,  1203,
    1217,  1231,  1245,  1260,  1274,  1289,  1304,  1319,  1334,  1350,  1366,  1381,  1397,  1414,  1430,
    1446,  1463,  1480,  1497,  1515,  1532,  1550,  1568,  1586,  1604,  1623,  1642,  1661,  1680,  1699,
    1719,  1739,  1759,  1780,  1800,  1821,  1842,  1863,  1885,  1907,  1929,  1951,  1974,  1997,  2020,
    2043,  2067,  2091,  2115,  2139,  2164,  2189,  2215,  2240,  2266,  2292,  2319,  2346,  2373,  2400,
    2428,  2456,  2485,  2514,  2543,  2572,  2602,  2632,  2663,  2693,  2725,  2756,  2788,  2820,  2853,
    2886,  2919,  2953,  2987,  3022,  3057,  3092,  3128,  3164,  3201,  3238,  3276,  3314,  3352,  3391,
    3430,  3470,  3510,  3551,  3592,  3633,  3675,  3718,  3761,  3805,  3849,  3893,  3938,  3984,  4030,
    4077,  4124,  4172,  4220,  4269,  4318,  4368,  4419,  4470,  4522,  4574,  4627,  4681,  4735,  4790,
    4845,  4901,  4958,  5015,  5073,  5132,  5192,  5252,  5313,  5374,  5436,  5499,  5563,  5627,  5692,
    5758,  5825,  5893,  5961,  6030,  6100,  6170,  6242,  6314,  6387,  6461,  6536,  6611,  6688,  6766,
    6844,  6923,  7003,  7084,  7166,  7249,  7333,  7418,  7504,  7591,  7679,  7768,  7858,  7949,  8041,
    8134,  8228,  8323,  8420,  8517,  8616,  8716,  8817,  8919,  9022,  9126,  9232,  9339,  9447,  9557,
    9667,  9779,  9892, 10007, 10123, 10240, 10359, 10479, 10600, 10723, 10847, 10972, 11099, 11228, 11358,
    11489, 11623, 11757, 11893, 12031, 12170, 12311, 12454, 12598, 12744, 12891, 13041, 13192, 13344, 13499,
    13655, 13813, 13973, 14135, 14299, 14464, 14632, 14801, 14973, 15146, 15321, 15499, 15678, 15860, 16044,
    16229, 16417, 16607, 16800, 16994, 17191, 17390, 17591, 17795, 18001, 18210, 18420, 18634, 18850, 19068,
    19289, 19512, 19738, 19966, 20198, 20431, 20668, 20907, 21149, 21394, 21642, 21893, 22146, 22403, 22662,
    22925, 23190, 23458, 23730, 24005, 24283, 24564, 24848, 25136, 25427, 25722, 26020, 26321, 26626, 26934,
    27246, 27561, 27880, 28203, 28530, 28860, 29194, 29532, 29874, 30220, 30570, 30924, 31282, 31645, 32011,
    32382, 32757, 33136, 33520, 33908, 34300, 34698, 35099, 35506, 35917, 36333, 36754, 37179, 37610, 38045,
    38486, 38931, 39382, 39838, 40300, 40766, 41238, 41716, 42199, 42687, 43182, 43682, 44188, 44699, 45217,
    45740, 46270, 46806, 47348, 47896, 48451, 49012, 49579, 50153, 50734, 51322, 51916, 52517, 53125, 53740,
    54363, 54992, 55629, 56273, 56925, 57584, 58251, 58925, 59607, 60298, 60996, 61702, 62417, 63139, 63870,
    64610, 65358, 66115, 66881, 67655, 68438, 69231, 70033, 70843, 71664, 72494, 73333, 74182, 75041, 75910,
    76789, 77678, 78578, 79488, 80408, 81339, 82281, 83234, 84198, 85173, 86159, 87157, 88166, 89187, 90219,
    91264, 92321, 93390, 94471, 95565, 96672, 97791, 98924,100069,101228,102400,103586,104785,105999,107226,
    108468,109724,110994,112279,113580,114895,116225,117571,118932,120310,121703,123112,124537,125980,127438,
    128914,130407,131917,133444,134989,136553,138134,139733,141351,142988,144644,146319,148013,149727,151461,
    153215,154989,156783,158599,160435,162293,164172,166073,167996,169942,171910,173900,175914,177951,180011,
    182096,184204,186337,188495,190678,192886,195119,197379,199664,201976,204315,206681,209074,211495,213944,
    216421,218927,221462,224027,226621,229245,231900,234585,237301,240049,242829,245640,248485,251362,254273,
    257217,260196,263209,266256,269339,272458,275613,278805,282033,285299,288602,291944,295325,298745,302204,
    305703,309243,312824,316446,320111,323817,327567,331360,335197,339078,343005,346976,350994,355059,359170,
    363329,
};
...
static HI_VOID cmos_again_calc_table(HI_U32 *pu32AgainLin, HI_U32 *pu32AgainDb)
{
    ...
    if (*pu32AgainLin >= gain_table[510])
    {
         *pu32AgainLin = gain_table[510];
         *pu32AgainDb = 510;
         return ;
    }
...
static HI_VOID cmos_gains_update(HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    g_stSnsRegsInfo.astSspData[4].u32Data = ((u32Again & 0x100) >> 8); 
    return;
}
...
static HI_VOID cmos_get_inttime_max(HI_U32 u32Ratio, HI_U32 *pu32IntTimeMax)
{
    ...
    if( (WDR_MODE_2To1_LINE == genSensorMode) && (SENSOR_QXGA_30FPS_MODE == gu8SensorImageMode) )
    {
        ...
        u32ShortIntTimeMax = ((gu32FullLinesStd - 22) * 0x40  - ((gu32FullLinesStd - 22) * 0x40%(DIV_0_TO_1(0x400 + 0x40)) )  ) / DIV_0_TO_1(0x400 + 0x40); 
        ...
        u32ShortIntTimeMax = (  (gu32FullLinesStd - 22) * 0x40  - ( (gu32FullLinesStd - 22) * 0x40%(DIV_0_TO_1(u32Ratio + 0x40)) )  ) / DIV_0_TO_1(u32Ratio + 0x40);
        ...
        if(u32ShortIntTimeMax <=  (gu32RHS1_LocRLimit - 11)  )
        {
            ...
            if(u32LongIntTimeMax <= (gu32FullLinesStd - gu32RHS1_LocRLimit - 11))     //keep the RHS1;
            {
            ...
            {
                gu32RHS1 = gu32FullLinesStd - u32LongIntTimeMax - 11;
...
HI_VOID cmos_set_pixel_detect(HI_BOOL bEnable)
{
    ...
    u32FullLines_5Fps = (u32FullLines_5Fps > 0x1FFFF) ? 0x1FFFF : u32FullLines_5Fps;
    ...
    switch(u8Mode)
    {
        case WDR_MODE_NONE:
            ...
            else if(SENSOR_1080P_60FPS_MODE== gu8SensorImageMode)
            {
                ...
                gu32FullLines = IMX123_VMAX_1080P_60FPS_LINEAR;
            }
            else
            {
        ...
        case WDR_MODE_2To1_LINE:
            ...
            if(SENSOR_QXGA_30FPS_MODE == gu8SensorImageMode)
            {
                ...
                gu32FullLines = IMX123_VMAX_QXGA_30FPS_DOL_WDR *2;
            }
            else
            {
        ...
        case WDR_MODE_2To1_FRAME:       //half rate
            ...
            else if(SENSOR_QXGA_60FPS_MODE == gu8SensorImageMode)
            {
                ...
                gu32FullLines = IMX123_VMAX_QXGA_60FPS_LINEAR;
            }
            else
            {
            }
