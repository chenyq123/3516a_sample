...
int main(int argc, char const *argv[])
{
    ...
    while(k--)
    {
        ...
        s32Ret = HI_MPI_SYS_MmzAlloc_Cached(&stDst.u32PhyAddr[0],(void**) &stDst.pu8VirAddr[0], "user", HI_NULL, u32BlkSize);
        HI_MPI_SYS_MmzFlushCache(stDst.u32PhyAddr[0], (void *)stDst.pu8VirAddr[0], u32BlkSize);
        ...
        stDst.u32PhyAddr[0] = HI_MPI_VB_Handle2PhysAddr(hBlock);
        stDst.pu8VirAddr[0] = (HI_U8*) HI_MPI_SYS_Mmap(stDst.u32PhyAddr[0], u32BlkSize);
        stDst.u16Stride[0] = FrameInfo.stVFrame.u32Stride[0] * 3;
        stDst.u16Width = FrameInfo.stVFrame.u32Width;
        stDst.u16Height = FrameInfo.stVFrame.u32Height;
#endif
        ...
        //stSrc.u32PhyAddr[2] = stSrc.u32PhyAddr[1] + 1;

        //stSrc.pu8VirAddr[0] = (HI_U8*)FrameInfo.stVFrame.pVirAddr[0];
        //stSrc.pu8VirAddr[1] = (HI_U8*)FrameInfo.stVFrame.pVirAddr[1];
        stSrc.pu8VirAddr[0] = (HI_U8*)HI_MPI_SYS_Mmap(FrameInfo.stVFrame.u32PhyAddr[0],u32BlkSize / 2);
        ...
        printf("pVirAddr[0]:%x\npVirAddr[1]:%x\npVirAddr[2]:%x\n",stDst.pu8VirAddr[0],stDst.pu8VirAddr[1],stDst.pu8VirAddr[2]);
