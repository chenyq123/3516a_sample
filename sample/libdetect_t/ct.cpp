	...
	for (int i=0; i<featureNum; i++)
	{
		for (int j=0; j<sampleBoxSize; j++)
		{
			...
			for (size_t k=0; k<features[i].size(); k++)
			{
				...
				if(yMin>=_imageIntegral.rows) yMin = _imageIntegral.rows-1;
				if(yMax>=_imageIntegral.rows) yMax = _imageIntegral.rows-1;
				if(xMin>=_imageIntegral.cols) xMin = _imageIntegral.cols-1;
				if(xMax>=_imageIntegral.cols) xMax = _imageIntegral.cols-1;
