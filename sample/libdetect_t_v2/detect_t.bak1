...
public:
	...
	void do_mask(Mat &img, Mat img_mask);// +++
	...
	bool ismask_;
	Mat img_mask_; // 标定区掩码图像;// +++
	bool is_up_mask_;// +++
	Mat upbody_img_mask_; // +++
...
protected:
	...
	std::vector < Rect > upbody_refineSegments2(Mat img, Mat & mask, Mat & dst, double interval, double marea, double mrect_area);

	//掩码;
	bool build_mask_internal(const char *key, Mat& img, Rect mask_rc);// +++
	bool build_mask(Mat &img_mask, Rect mask_rc, const char *key, const char *key2 = 0);// +++
	...
	cv::Rect get_rect(Rect init_rect, const char* pt1, const char* pt2 = 0); // +++
	void init_mask( );
};
