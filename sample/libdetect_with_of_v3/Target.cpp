    ...
    double stamp, double min_dis_5frames, int min_pts, int max_feature_pts, double qualitylevel)
{
    ...
    int max_features_pts = atoi(cfg->get_value("pd_max_features_pts", "200"));

    PTS pts;
    //cv::goodFeaturesToTrack(curr_gray(roi), pts, max_feature_pts, qualitylevel, 1.5);
    printf("good begin, time:%d\n",GetTickCount());
    hi_goodFeaturesToTrack(curr_gray(roi), pts, max_feature_pts, qualitylevel, 1.5);
    printf("good end, time:%d\n",GetTickCount());
...
bool Target::track(const cv::Mat &prev, const cv::Mat &curr, double stamp)
{
    ...
    return update(curr_pts, stamp, search_roi);
}

bool Target::update(const std::vector<cv::Point2f> &curr_pts, double stamp, const cv::Rect &search_roi)
{
    /// 删除错误点对应的轨迹 ...
    PTS valid_pts;
