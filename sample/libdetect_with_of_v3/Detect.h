...
public:
    ...
    {
        cv::Rect pos;           // 站立位置
        double stamp_begin;     // 起始时间

        bool enable_od;         // 是否启用了识别
        cv::Rect obj_pos;       // 如果启用了识别，检测到目标的位置 ..
    };
    ...
    virtual void detect(const cv::Mat &origin, std::vector<STANDUP> &standups);
...
protected:
    ...
    /// 是否支持内部维护“站立状态”，如果支持，则将调用 detect2，而不是 detect
    ...
    bool build_mask(cv::Mat &mask)
    {
        ...
        if (points.size() > 3) {
            ...
            mask = cv::Mat::zeros(cv::Size(atoi(cfg_->get_value("video_width", "480")),
                atoi(cfg_->get_value("video_height", "360"))), CV_8UC3);
