...
void hi_CopyDataToIveImageU8C1(Mat Src, IVE_IMAGE_S *pstDst)
{
    if(Src.data == NULL || pstDst->pu8VirAddr == NULL)
        return ;
    printf("height:%d, width:%d, cols:%d, rows:%d, step:%d\n", pstDst->u16Height, pstDst->u16Width, Src.cols, Src.rows, Src.step[0]);
...
void hi_dilate(Mat Src, Mat &Dst, Mat element, Point anchor, int iterations)
{
    ...
    HI_U8 mask5x5[25] = {255, 255, 255, 255, 255,
                         255, 255, 255, 255, 255,
                         255, 255, 255, 255, 255,
                         255, 255, 255, 255, 255,
                         255, 255, 255, 255, 255};
   ...
   // printf("line=%d,time=%d\n",__LINE__,GetTickCount1());
    ...
    memcpy(stDilateCtrl.au8Mask, mask5x5, sizeof(HI_U8) * 25);
    pSrc = &stSrc;
    pDst = &stDst;
...
void hi_erode(Mat Src, Mat &Dst, Mat element, Point anchor, int iterations)
{
    ...
    memcpy(stErodeCtrl.au8Mask, mask5x5, sizeof(HI_U8) * 25);
    pSrc = &stSrc;
    pDst = &stDst;
    for(int i = 0; i < iterations; i++)
    {
...
void calcLKOpticalFlow(cv::Mat pre_gray, cv::Mat cur_gray, vector<Point2f> &prepoint, vector<Point2f> &nextpoint, vector<uchar> &state)
{
    ...
    {
        s32DownIndex /= 2;
        ret = hi_CreateIveImageU8C1(&stSrcPre[s32Index],
                (width % 2) == 0 ? width / s32DownIndex : width / s32DownIndex + 1,
                (height % 2) == 0 ? height / s32DownIndex : height / s32DownIndex + 1);
        if(ret != HI_SUCCESS)
        {
        ...
        ret = hi_CreateIveImageU8C1(&stSrcCur[s32Index],
                (width % 2) == 0 ? width / s32DownIndex : width / s32DownIndex + 1,
                (height % 2) == 0 ? height / s32DownIndex : height / s32DownIndex + 1);
        if(ret != HI_SUCCESS)
        {
