...
public:
    ...
    bool update(const std::vector<cv::Point2f> &to, double stamp, const cv::Rect &roi); // to 为 last_pts 的当前位置 ..
    ...
    /// 返回最新的特征点 ...
    inline const std::vector<cv::Point2f> &last_pts() const
    {
        if (layers_.empty()) {
            //throw new std::exception("??? NEVER reach here ???");
        }
        else {
            return layers_.back();
        }
