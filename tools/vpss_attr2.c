...
#define USAGE_HELP(void)\
    do{\
        ...
        printf("\t\tsfc             [空域滤波强度，value:0~255,   default:8]\n");   \
        printf("\t\ttfc             [时域滤波强度，value:0~32, default:0]\n");   \
        printf("\t\tiepost          [纹理/边缘是否后置处理，value:0~1,  default:0]\n");   \
        printf("\t\tie              [纹理/边缘增强强度，value:0~63,  default:63]\n");   \
        printf("\t\tysfm 	        [亮度运动区域空域亮区强度，value:0~255, default:32]\n");   \
        printf("\t\tysfs 	        [亮度静止区域空域亮区强度，value:0~64, default:32]\n");   \
        printf("\t\tytf  	        [亮度时域强度，value:0~31, default:31]\n");   \
        printf("\t\tymth            [亮度的运动检测阈值，value:0~511, default:64]\n");\
        printf("\t\tdesand          [去颗粒噪声，value:0~8, default:0 ]\n");\
    }while(0)
